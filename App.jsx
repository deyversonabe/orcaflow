...
// Import remote store to sync data via Supabase across devices
import { store as remoteStore } from "./store";
// Import helper hook to detect mobile devices and handle responsive layouts
...
const store = {
  // Delegate storage to Supabase-based store for cross-device sync
  async get(key) {
...
function Row({ children }) {
  return (
    <div
      style={{
        display: "grid",
        // Use auto-fit with minmax to allow the row to responsively stack
        // its children on narrow screens and use two columns on wider screens.
...
function DashboardPanel({ crm, empresas, meta, usuarioAtual, setView }) {
  ...
  // Determine mobile layout at render time
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 22, position: "relative", zIndex: 2 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.15fr .85fr", gap: 16, alignItems: "stretch" }}>
...
function GestaoPage({ crm = [], setCrm, empresas = [], meta = {}, pushToast, usuarioAtual, setView, abrirOrcamentoSalvo, baixarOrcamento, onAnexar }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: 22, position: "relative", zIndex: 2 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.2fr .8fr", gap: 16, marginBottom: 18 }}>
...
export default function App() {
  ...
  // Detect if the current viewport is considered mobile to adjust layouts
  const isMobile = useIsMobile();

  // Ensure the viewport meta tag is set for responsive behavior on mobile devices
  useEffect(() => {
  ...
  // Listen for remote changes across devices and update local state.
  // When another device modifies CRM or user data, the Supabase-powered
  // store emits a notification. We register a callback to update the
  // relevant React state in real time. The returned function cleans up
  // the listener when this component unmounts.
  useEffect(() => {
    const off = remoteStore.onRemoteChange((key, value) => {
      // Normalize missing values to empty arrays for easier consumption.
      if (key === KEY_CRM) setCrm(value || []);
      if (key === KEY_USERS) setUsuarios(value || []);
    });
    return () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: `radial-gradient(circle at 0% 0%, ${BRAND.green2}12, transparent 28%), radial-gradient(circle at 100% 5%, ${BRAND.blue2}12, transparent 28%), ${BRAND.bg}`, color: BRAND.text, fontFamily: "'Segoe UI', system-ui, sans-serif", display: "flex", flexDirection: "column" }}>
      <style>{`
        ...
        /* Mobile adjustments: hide watermark and tweak font sizes for better readability */
        @media (max-width: 768px) {
          .of-watermark { display: none; }
          h1.of-title-gradient { font-size: 24px !important; }
      ...
      {view === "orcamento" && (
        <div style={{ flex: 1, display: "flex", overflow: "hidden", flexDirection: isMobile ? "column" : "row" }}>
          <div style={{ width: isMobile ? "100%" : 262, background: "rgba(10,20,32,.94)", borderRight: isMobile ? undefined : `1px solid ${BRAND.border}`, display: "flex", flexDirection: "column", flexShrink: 0 }}>
