...
// 7) Melhorias: exportação CSV do CRM, salvamento de edição no histórico,
//    valor BR corrigido na seleção e proteção extra de armazenamento.
// ─────────────────────────────────────────────────────────────────────────────
...
function useDB() {
  ...
  const salvarEmpresa = useCallback(
    async (form) => {
      ...
      if (storageSizeOf(nova) > STORAGE_SOFT_LIMIT_BYTES) {
...
function DashboardPanel({ crm, empresas, meta, usuarioAtual, setView }) {
  ...
  const valorPotencial = lista.filter((o) => o.status !== "Finalizado").reduce((acc, o) => acc + parseValorBR(o.valorGlobal ?? o.valor), 0);
  const totalValor = lista.reduce((acc, o) => acc + parseValorBR(o.valorGlobal ?? o.valor), 0);
...
function GestaoPage({ crm = [], setCrm, empresas = [], meta = {}, pushToast, usuarioAtual, setView, abrirOrcamentoSalvo, baixarOrcamento, onAnexar }) {
  ...
  const isAdmin = usuarioAtual?.tipo === "admin";
  const base = useMemo(() => (isAdmin ? crm : crm.filter((o) => o.userId === usuarioAtual?.id)), [crm, isAdmin, usuarioAtual?.id]);

  const lista = useMemo(() => base.filter((o) => {
    const textoBusca = [
      o.cliente,
      o.empresaNome,
      o.empresa,
    ...
    return okBusca && okStatus && okEmpresa;
  }), [base, busca, statusFiltro, empresaFiltro]);
  ...
  const exportarCSV = () => {
    ...
    const linhas = lista.map((item) => [
      item.numero || "",
      item.cliente || "",
      item.empresaNome || item.empresa || "",
      brl(item.valorGlobal ?? item.valor),
      statusReal(item),
      item.proximoContato || "",
      item.lembreteIA || item.lembrete || "",
      tsFmt(item.criadoEm),
      tsFmt(item.atualizadoEm),
    ]);

    const csv = [cabecalho, ...linhas].map((linha) => linha.map(csvEscape).join(";")).join("\n");
    baixarTexto(`orcaflow-crm-${new Date().toISOString().slice(0, 10)}.csv`, `\uFEFF${csv}`, "text/csv;charset=utf-8");
    pushToast("Relatório CSV exportado com os filtros atuais.", "ok");
  };
    ...
    <div style={{ flex: 1, overflowY: "auto", padding: 22, position: "relative", zIndex: 2 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button onClick={exportarCSV} style={{ padding: "12px 18px", borderRadius: 14, border: `1px solid ${BRAND.border2}`, background: "transparent", color: BRAND.muted, fontWeight: 900, cursor: "pointer" }}>
            ⬇ Exportar CSV
          </button>
  ...
  const fieldChange = (empId, campo, val) => {
    setOrcamentos((prev) => {
      const atualizado = {
        ...
        [empId]: {
          ...prev[empId],
          campos: { ...prev[empId]?.campos, [campo]: val },
        },
      };

      setCrm((listaAtual) => {
        const novaLista = listaAtual.map((item) => {
          if (item.empresaId !== empId || item.orcamentoCompleto?.numero !== atualizado[empId]?.numero) return item;
          return {
            ...item,
            cliente: campo === "cliente" ? val : item.cliente,
            atualizadoEm: new Date().toISOString(),
            orcamentoCompleto: atualizado[empId],
          };
        });
        store.set(KEY_CRM, novaLista);
        return novaLista;
              ...
              {empresas.length === 0 ? <div style={{ padding: "22px 8px", textAlign: "center" }}><div style={{ fontSize: 26, opacity: 0.2, marginBottom: 8 }}>🏢</div><div style={{ fontSize: 11, color: BRAND.dim, lineHeight: 1.6, marginBottom: 11 }}>Nenhuma empresa cadastrada</div><button onClick={() => setView("empresas")} style={{ padding: "6px 13px", borderRadius: 7, border: `1px solid ${BRAND.green2}55`, background: "transparent", color: BRAND.green, cursor: "pointer", fontSize: 11, fontWeight: 850 }}>Cadastrar →</button></div> : empresas.map((emp) => {
                ...
                return (
                  <div key={emp.id} style={{ marginBottom: 8, borderRadius: 11, overflow: "hidden", border: `2px solid ${isSel ? emp.corPrimaria || BRAND.green2 : BRAND.border2}`, opacity: bloq ? 0.4 : 1, transition: "all .22s ease" }}>
                    ...
                    {isSel && <div style={{ padding: "8px 10px", background: `${emp.corPrimaria || BRAND.green2}0c`, borderTop: `1px solid ${emp.corPrimaria || BRAND.green2}22` }}><div style={{ fontSize: 8, fontWeight: 900, color: emp.corPrimaria || BRAND.green2, letterSpacing: 1.3, marginBottom: 5 }}>VALOR GLOBAL (R$)</div><input type="text" inputMode="decimal" value={s.valorGlobal} onChange={(e) => setValor(emp.id, e.target.value)} placeholder="0,00" style={{ width: "100%", background: BRAND.bg, border: `1px solid ${emp.corPrimaria || BRAND.green2}55`, borderRadius: 6, padding: "6px 8px", color: BRAND.text, fontSize: 13, fontWeight: 900, outline: "none", boxSizing: "border-box" }} />{s.valorGlobal && parseValorBR(s.valorGlobal) > 0 && <div style={{ fontSize: 10.5, color: emp.corPrimaria || BRAND.green2, marginTop: 4, fontWeight: 800 }}>→ {brl(s.valorGlobal)}</div>}</div>}
                  </div>
                );
              })}
            </div>
