...
function resumoClientes(clientes = []) {
  ...
  return clientes.slice(0, 25).map((cliente) => {
    ...
    return {
      ...
      email: clean(cliente?.email, 160),
      email2: clean(cliente?.email2, 160),
      telefone: clean(cliente?.telefone, 80),
      whatsapp: clean(cliente?.whatsapp, 80),
      telefone2: clean(cliente?.telefone2, 80),
      documento: clean(cliente?.documento, 80),
      endereco: clean(cliente?.endereco, 180),
      cidadeUf: clean(cliente?.cidadeUf, 100),
