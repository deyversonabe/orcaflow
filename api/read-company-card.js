...
function montarPrompt({ texto, filename }) {
  return `
Voce e um extrator de dados do Cartao CNPJ brasileiro.

TAREFA:
Ler o texto extraido do PDF oficial do Cartao CNPJ e devolver os dados cadastrais da empresa em JSON.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (rejectOversizedRequest(req, res, 900_000)) return;

    if (!process.env.OPENAI_API_KEY) {
      return res.status(500).json({ error: "OPENAI_API_KEY nao configurada no Vercel." });
    }

    const { texto, filename = "" } = req.body || {};
    const safeText = clean(texto, 50000);

    if (!safeText || safeText.length < 40) {
      return res.status(400).json({ error: "Texto insuficiente extraido do PDF." });
    }
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify(montarPayloadOpenAI(montarPrompt({ texto: safeText, filename }))),
    });
